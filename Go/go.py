...
def set_board_size(command):
    # hook global variables | các biến toàn cục
    global BOARD_WIDTH, BOARD_RANGE, board, coords
    
    # parse the board size | phân tích kích thước bảng
    size = int(command.split()[-1])
    
    # throw error if board size is not supported | ném lỗi nếu kích thước bảng không được hỗ trợ
    if size not in [9, 13, 19]:
        print('? current board size not supported\n')
        return

    # calculate current board size | tính toán kích thước bảng hiện tại
    BOARD_WIDTH = size
    BOARD_RANGE = BOARD_WIDTH + MARGIN
    board = BOARDS[str(size)]
    coords = COORDS[str(size)]

# count liberties, save stone group coords | đếm quyền tự do, lưu hợp âm nhóm đá
def count(square, color):
    # init piece | mảng ban đầu
    piece = board[square]
    
    # skip offboard squares | bỏ qua các ô vuông ngoài khơi
    if piece == OFFBOARD: return
    
    # if there's a stone at square | nếu có một hòn đá ở hình vuông
    if piece and piece & color and (piece & MARKER) == 0:
        # save stone's coordinate | lưu tọa độ của đá
        block.append(square)
        
        # mark the stone | đánh dấu đá
        board[square] |= MARKER
        
        # look for neighbours recursively | tìm kiếm hàng xóm đệ quy
    ...
    # if the square is empty | nếu hình vuông trống
    elif piece == EMPTY:
        # mark liberty | đánh dấu tự do
        board[square] |= LIBERTY
        
        # save liberty | lưu tự do
        liberties.append(square)

# remove captured stones | loại bỏ đá bị bắt
def clear_block():
    for captured in block: board[captured] = EMPTY

# clear groups | Làm sạch
def clear_groups():
    # hook global variables | móc các biến toàn cục
    global block, liberties
    
    # clear block and liberties lists | xóa danh sách khối và quyền tự do
    block = []
    liberties = []

# restore the board after counting stones | khôi phục bảng sau khi đếm đá
def restore_board():
    # clear groupd | Làm sạch
    clear_groups()
    
    # unmark stones | bỏ đánh dấu đá
    for square in range(BOARD_RANGE * BOARD_RANGE):
        # restore piece if the square is on board | khôi phục quân cờ nếu hình vuông nằm trên bàn cờ
        if board[square] != OFFBOARD: board[square] &= 3

# clear board | Làm sạch
def clear_board():
    # clear groupd | Làm sạch
    clear_groups()
...
# make move on board | di chuyển trên tàu
def set_stone(square, color):
    # make move on board | di chuyển trên tàu
    board[square] = color
    
    # handle captures | xử lý ảnh chụp
    captures(3 - color)

# generate random move | tạo di chuyển ngẫu nhiên
def make_random_move(color):
    # find empty random square | tìm hình vuông ngẫu nhiên trống
    ...
    # count liberties | đếm quyền tự do
    count(random_square, color)
    
    # suicide move | di chuyển tự sát
    if len(liberties) == 0:
        # restore board | khôi phục bảng
        restore_board()
        
        # take off the stone | lấy đá ra
        board[random_square] = EMPTY
        
        # search for another move | tìm kiếm một động thái khác
        try:
            # return non suicide move | trở lại di chuyển không tự sát
            return make_random_move(color)
        except:
            # pass the move | bỏ qua nước đánh 
            return '' 
    
    # restore board
    restore_board()
    
    # print random move | in di chuyển ngẫu nhiên
    eprint('random move:', coords[random_square])
    
    # return the move | trả lại di chuyển
    return coords[random_square]

# play command | lệnh chơi
def play(command):
    # parse color | phân tích màu sắc
    color = BLACK if command.split()[1] == 'B' else WHITE

    # handle GUI pass move | xử lý di chuyển qua GUI
...
def genmove(color):
    ...
    if best_move:
        ...
        if not legal:
            ...
            while fix_random == coords[pattern] or fix_random == coords[surround] or fix_random == coords[defend] or fix_random == coords[save]:
                fix_random = make_random_move(color)
                cnt = cnt+1
                if cnt == 1:
                    fix_random = 'end'
                    return fix_random  
...
def gtp():
    ...
    while True:
        ...
        if 'name' in command: print('= G17\n')
            ...
            remid = genmove(BLACK if command.split()[-1] == 'B' else WHITE)
            if remid == 'testcase':
             pyautogui.hotkey('ctrl', 'p')
            if remid == feedback:
                remid = make_random_move(BLACK if command.split()[-1] == 'B' else WHITE)
                print('=',remid+'\n')
                feedback = 'abc'
            else : 
                feedback = remid 
